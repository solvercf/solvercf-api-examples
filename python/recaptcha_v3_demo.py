...
WEBSITE_KEY = rc["websiteKey"]
PAGE_ACTION = rc.get("pageAction", "demo_page")
VERIFY_URL = rc["verifyUrl"]
API_BASE_URL = "https://solvercf.com/token/extension"

def create_task(client_key, website_url, website_key, page_action="demo_page", session=None):
...
def main():
    print("=" * 60)
    print("  🚀 SolverCF API - Google reCAPTCHA v3 Demo")
    print("=" * 60)
    ...
    try:
        res = create_task(CLIENT_KEY, WEBSITE_URL, WEBSITE_KEY, PAGE_ACTION, session=session)
    ...
    if isinstance(raw_json_str, str):
        try:
            raw_details = json.loads(raw_json_str)
            formatted_res["rawJson"] = raw_details
        except ValueError:
            pass
    elif isinstance(raw_json_str, dict):
        raw_details = raw_json_str

    print("\nVerify Response:")
    print(json.dumps(formatted_res, indent=2))

    score = raw_details.get("score", verify_res.get("score"))
    hostname = raw_details.get("hostname", verify_res.get("hostname"))
    action = raw_details.get("action", verify_res.get("action"))

    print("-" * 60)
    if verify_res.get("success"):
        score_text = f" (Score: {score})" if score is not None else ""
        print(f"[🎉 SUCCESS] reCAPTCHA v3 verified successfully!{score_text}")
        if hostname or action:
            print(f"[📊 Result] Score: {score} | Action: {action} | Host: {hostname}")
    else:
